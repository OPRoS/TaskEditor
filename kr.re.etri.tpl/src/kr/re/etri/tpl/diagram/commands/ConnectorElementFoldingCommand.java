...
public class ConnectorElementFoldingCommand extends Command {
	/** 새로운 크기와 위치 */
	 ...
	 */
	public ConnectorElementFoldingCommand(ReferElement model,
	...
	public void redo() {
		...
		if(shapeModel.isCollapsed() == false) {
			...
			ConnectorElement connector = (ConnectorElement)shapeModel.getRealModel();
			for(ItemElement item : itemList) {
				if(item instanceof LinkedElement) {
					...
					for(ConnectionElement withCon : itemConns) {
						ReferElement refItem = (ReferElement)withCon.getTarget();
						if(refItem.getRealModel() instanceof WithElement) {
							withCon.setVisible(ConnectorType.SEQEXER == connector.getConType());
						}
					}
			...
			for(ItemElement item : itemList) {
				if(item instanceof LinkedElement) {
					...
					for(ConnectionElement con : itemConns) {
						...
						ItemElement target = refItem.getRealModel();
						if(target instanceof WithElement) {
							con.setVisible(ConnectorType.SEQEXER == connector.getConType());
						}
					}
