...
public class ItemPropertySheetEntry extends EventManager implements IPropertySheetEntry {
	/**
	...
	private ItemPropertySheetEntry parent;
	...
	private ItemPropertySheetEntry[] childEntries = null;
	...
	private ICellEditorListener cellEditorListener = new ICellEditorListener() {
		...
		public void applyEditorValue() {
			ItemPropertySheetEntry.this.applyEditorValue();
		}
	};
	...
	private void createChildEntries() {
		...
		ItemPropertySheetEntry[] newEntries = new ItemPropertySheetEntry[descriptors
				.size()];
		for (int i = 0; i < descriptors.size(); i++) {
			IPropertyDescriptor d = (IPropertyDescriptor) descriptors.get(i);
			// create new entry
			ItemPropertySheetEntry entry = createChildEntry();
	...
	/**
	 * Factory method to create a new child <code>ItemPropertyEntry</code>
	 * instance.
	 * <p>
	 * Subclasses may overwrite to create new instances of their own class.
	 * </p>
	 * 
	 * @return a new <code>ItemPropertyEntry</code> instance for the
	 *         descriptor passed in
	 * @since 3.1
	 */
	protected ItemPropertySheetEntry createChildEntry() {
		return new ItemPropertySheetEntry();
	}
	...
	public void dispose() {
		...
		ItemPropertySheetEntry[] entriesToDispose = childEntries;
		childEntries = null;
	 ...
	 * @since 3.1
	 */
	protected ItemPropertySheetEntry getParent() {
		return parent;
	}
	...
	public IPropertySource getPropertySource(int index) {
		// get our parent's value objects
		Object[] currentSources = parent.getValues();
		if(index < 0 || currentSources.length <= index) {
			return null;
		}

		return getPropertySource(currentSources[index]);
	}
	...
	private void refreshChildEntries() {
		...
		for (int i = 0; i < childEntries.length; i++) {
			ItemPropertySheetEntry childEntry = childEntries[i];
			if (childEntry != null) {
		...
		for (int i = 0; i < descriptors.size(); i++) {
			...
			ItemPropertySheetEntry entry = (ItemPropertySheetEntry) entryCache.get(d
					.getId());
			if (entry != null) {
				// reuse old entry
				entry.setDescriptor(d);
	 ...
	 */
	private void setParent(ItemPropertySheetEntry propertySheetEntry) {
	...
	/**
	 * The <code>ItemPropertyEntry</code> implmentation of this method
