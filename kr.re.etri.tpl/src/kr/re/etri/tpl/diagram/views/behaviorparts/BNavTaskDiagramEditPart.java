 ...
 */
public class BNavTaskDiagramEditPart extends BNavItemElementEditPart implements Adapter {
	 ...
	 */
	public BNavTaskDiagramEditPart(ModelDiagram model) {
		super(model);
	}
...
//		}
	...
	protected List<ItemElement> getTaskChildren(IncludedElement parentItem) {
		ArrayList<ItemElement> actionList = new ArrayList<ItemElement>();

		List<ItemElement> itemList = parentItem.getItems();
		for(ItemElement item : itemList) {
			if(item instanceof IncludedElement) {
				actionList.addAll(getTaskChildren((IncludedElement)item));
			}
...
//		super.addChild(createChild(newObject), -1);
		...
		else if(newObject instanceof BehaviorElement) {
			super.addChild(createChild(newObject), -1);
		}
	}
