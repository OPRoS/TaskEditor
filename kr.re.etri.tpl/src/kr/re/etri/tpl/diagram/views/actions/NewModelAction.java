...
public class NewModelAction extends Action {
	...
	public final static String actionId = "kr.re.etri.tpl.views.SymbolNavigatorViewPage.NewModelAction";
	
	public NewModelAction(ItemElement rootModel, CommandStack cmdStack) {
		this("&New Model", rootModel, cmdStack);
	}

	public NewModelAction(String text, ItemElement rootModel, CommandStack cmdStack) {
		super(text);
	...
	public NewModelAction(String text, ImageDescriptor image) {
		super(text, image);
	}

	public NewModelAction(String text, int style) {
		super(text, style);
	}
	...
	public void run() {
		...
		request.setFactory(new CreationFactory() {
			public Object getNewObject() { return ModelManager.getFactory().createModelElement(); }
