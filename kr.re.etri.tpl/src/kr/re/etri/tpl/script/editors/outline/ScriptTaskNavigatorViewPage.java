...
public class ScriptTaskNavigatorViewPage extends ContentOutlinePage implements
		ITaskNavigatorViewPage, IDoubleClickListener {
	private static Logger logger = Logger.getLogger(ScriptTaskNavigatorViewPage.class);
	private ITextEditor editor;
	private IEditorInput input;
	private TaskOutlineContentProvider outlineContentProvider;
	private OutlineLabelProvider2 outlineLabelProvider;
	
	public ScriptTaskNavigatorViewPage(ITextEditor editor, IDocument doc) {
		super();
		this.editor = editor;
	}
	...
	public void createControl(Composite parent) {
		...
		outlineContentProvider = new TaskOutlineContentProvider(editor.getDocumentProvider());
	...
	public void dispose() {
		super.dispose();
//		System.out.println("ScriptModelNavigaorViewPage Disposed");
	}
		...
		if (selection.isEmpty()){
			...
			try {
				...
				if(element != null){
					...
					if(start >= 0 && length >= 0){
						((TPLScriptEditor)editor).removeSelectionChangedListener();
						editor.selectAndReveal(start, length);
						((TPLScriptEditor)editor).addSelectionChangedListener();
					}
				}
