...
public class ModelOutlineContentProvider implements ITreeContentProvider {
//	private static Logger logger = Logger.getLogger(ModelOutlineContentProvider.class);
	private IScriptNode root = null;
	private IEditorInput input;
	private IDocumentProvider documentProvider;
	private IDocumentProvider oldDocumentProvider;
	...
	public ModelOutlineContentProvider(IDocumentProvider provider) {
		super();
	...
	public void setDocumentProvider(IDocumentProvider provider) {
		this.oldDocumentProvider = this.documentProvider;
		this.documentProvider = provider;
	}
			...
			for (IScriptNode element : children) {
				if (RFSMParser.RTDL == element.getType()) {
					for (IScriptNode child : element.getChildren()) {
						if (RFSMParser.ENUM == child.getType() ||
								RFSMParser.MODEL == child.getType()) {
...
//					elements.add(element);
			...
			for(IScriptNode element : children){
				if(RFSMParser.ENUM == element.getType())
					elements.add(element);
			}
			for(IScriptNode element : children){
				if(RFSMParser.ELEM == element.getType())
					elements.add(element);
			}
			for(IScriptNode element : children){
				if(RFSMParser.VAR == element.getType())
					elements.add(element);
			}
			for(IScriptNode element : children){
				if(RFSMParser.MODEL == element.getType())
					elements.add(element);
			}
			for(IScriptNode element : children){
				if(RFSMParser.FUNC == element.getType())
					elements.add(element);
			}
			for(IScriptNode element : children){
				if(RFSMParser.PARM == element.getType())
					elements.add(element);
			}
